...
- workspace에 프로젝트가 생긴다.
- workspace를 재설정하면 설정들은 초기화된다.
- tomcat server를 설정하면 severs 폴더가 생긴다.
- .metadata는 이클립스 설정이 저장된다. 앞에 .이 붙은 폴더는 숨긴파일이다.
- git init을 하면 프로젝트가 동일한 저장소에 들어간다.
- .classpath .project .class /.setting 과 같은것은 개인설정이므로 커밋을 하지 않는다.

*https://www.toptal.com/developers/gitignore* (ignore setting)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
- 개인설정파일은 커밋을 하지 않는다.
![image](https://user-images.githubusercontent.com/108508922/190037464-1b51ba5a-c08e-4f5c-97f3-4befc4dd384e.png)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
### spring 8 - error 
#### 수업 요약
- DataAccessException
   1. SqlException=> DataAccessException (DB관련 예외가 발생하면 DAE로 change되서 발생한다)
- NotDeliveryExcepion
   1. 
- Transaction 
   1. pom.xml 에 spring-tx dependency 추가
	2. servlet-context.xml 에 transaction (datasource.DataSourceTransactionManager  ref="dataSource"로 bean설정을 해주어야한다.)
   3. @Transaction 어노테이션으로 트렌젝션관리 되도록한다.
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
- 에러 처리과정
![image](https://user-images.githubusercontent.com/108508922/190290342-8d7477df-09b0-4089-a886-7b6fca0b6dbf.png)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
### javascript jquery 
#### 수업 요약

1. $()함수를 통하여 $("selector").action().action2().action3()...와 같이 사용할 수 있다.
2. 
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
### javascript jquery2 
...
#### 수업 요약
- [node js]
1. sever side javascript
2. 원래 javascript는 웹브라우저의 동작을 프로그래밍 하는 언어이다. javascript를 해석해주는 실행환경(runtime)은 웹브라우저에 들어 있었다.
3. 이 실행환경을 window, mac os, linux 환경으로 구현 가능하게 해주는게 node js 이다.
4. node js가 설치 되어있다면 javascript로 응용프로그램을 개발 할 수 있게 해준다.
5. 아직까지 gui는 없다.
6. node.js 로 주로 무엇을 하는가?
	1. 웹서버를 만들거나 어떤 개발환경을 주로 제공한다.
7. 왜 node.js를 설치 하려 하는가.
	1. vue나 react는 node.js 환경에서 개발할 수 있도록 셋팅되어 있다.
8. node.js를 설치 한다고 해서 방대한 모든 기능이 한번에 설치되는 것은 아니다.
	1. 기본기능(package)만 설치가 되고 필요시 추가 package 를 설치 해야한다.
9. package를 추가 제거 관리 해주는 응용 프로그램은 npm(node package manager) 이다.


- 설치
1. nodejs.org 페이지에 접속한다.
2. other downloads<br>
![image](https://user-images.githubusercontent.com/108508922/190938758-d34d8f43-8ba5-4026-8fa5-35adacb49e01.png)<br>
3. all download option<br>
![image](https://user-images.githubusercontent.com/108508922/190938831-7201d870-7413-4d90-beaa-0f64abcde9ce.png)<br>
4. 상위목록으로 이동<br> 
![image](https://user-images.githubusercontent.com/108508922/190938906-53cb9701-0d5f-4844-b97d-fe5691f4f94f.png)<br>
5. 12version<br>
![image](https://user-images.githubusercontent.com/108508922/190938946-77c0ecb0-bb59-42d3-abaa-49a152461b22.png)<br>
6. 12-x64.msi 클릭하여 설치<br>
![image](https://user-images.githubusercontent.com/108508922/190939011-c2afe466-1af9-45d2-84f3-4aaccc0c0206.png)<br>
7. cmd에서 설치 확인하기<br>
![image](https://user-images.githubusercontent.com/108508922/190939031-8fb80b21-711a-4db6-aa8b-03944bfd3d1c.png)<br>
...
### javascript nodejs3
#### 수업 요약
- nmp run serve 작동시 일어나는 일.
1. node js web server run (node js web server가 실행된다)
2. vue dev activate (vue 작업환경이 시작된다)

- vue개발환경해석
1. public/index.html의 파일을 node js server 가 해석하여 웹브라우저에 로딩이 된다. 
2. src/main.js 가 main mathod 역할을 한다. server를 run하면 실행된다.(root component)

- nmp run build
1. 해당 폴더에 dist라는 폴더가 생긴다
2. compile의 결과가 생긴것이다. 
3. compile의 결과물을 spring 이나 다른 개발환경에 추가하여 사용한다.
4. tomcat 서버에서 사용하는 경로를 추가해준다.(사진)
...
- 파이썬은 함수 지향형 언어이다.
- raw string 을 선행하면 이스케이프 기능 해제
- 기본적으로 라인스킵을 가진다. 라인스킵을 원하지 않을 시에 end=''를 추가한다
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
### Python 7
#### 수업요약
- Socket
- secure coding 가이드 읽기
- https://www.mois.go.kr/frt/bbs/type001/commonSelectBoardArticle.do%3Bjsessionid=fr7QaTyG2gK5o02XJnYETp3havIQ1MGLKMYdWaaEe5me9IOk932SIy2BbP1AM08Z.mopwas54_servlet_engi
ne1?bbsId=BBSMSTR_000000000012&nttId=42152
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
### Python 8
#### 수업요약
Thread
sys.stdout.flush
- 파이썬에서는 GIL(global interpreter lock) 정책에 의해 완전한 스레드 구현은 불가능하다, 따라서 multiprocessing 모듈로 GIL정책을 우회하여 병렬처리가 가능하도록 하고 있다.
https://it-eldorado.tistory.com/160
- Pool: 입력 값에 대해 process들을 건너 건너 분배하여 함수 실행을 병렬 처리 한다.
- Process:
...
카이제곱검정
- 범주형 자료를 가지고 빈도를 검증(적합도 검정, 선호도 검정, 집단이 두개일 땐 동일성 검정) <br>
- 양수값을 가지며 오른쪽으로 긴꼬를 가지는 형태
- 카이제곱 통계량 수식
![image](https://user-images.githubusercontent.com/108508922/199899066-e4a31926-cd78-4426-8df1-009bf3d708f0.png)
- 기대값
- (각행의 주변값 x 각열의 주변값) / 전체 표본수
